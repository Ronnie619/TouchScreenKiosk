...
//            Vector3 worldPossDiff = go.transform.TransformPoint(posDiff);        
        ...
        public static Mesh Clone(Mesh mesh)
        {
            ...
            if(colors32 != null && colors32.Length > 0)
                output.colors32 = (Color32[])colors32.Clone();
            if(uv != null && uv.Length > 0)
                output.uv = (Vector2[])uv.Clone();
            if(uv2 != null && uv2.Length > 0)
                output.uv2 = (Vector2[])uv2.Clone();
			#if !UNITY_4_6
            if(uv3 != null && uv3.Length > 0)
                output.uv3 = (Vector2[])uv3.Clone();
            if(uv4 != null && uv4.Length > 0)
                output.uv4 = (Vector2[])uv4.Clone();
			#endif
            if(normals != null && normals.Length > 0)
                output.normals = (Vector3[])normals.Clone();
            if(tangents != null && tangents.Length > 0)
                output.tangents = (Vector4[])tangents.Clone();
